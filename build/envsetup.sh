...
function func_ccache()
{
    ...
    if [[ "${ccache_use}" == "" || "${ccache_use}" == "0" || "${ccache_use}" == "false" ]]; then echo -e "${CL_GRN} * Disabled ccache${CL_RST}\n"; export USE_CCACHE=0; else export USE_CCACHE=1; fi
    if [ "${USE_CCACHE}" == "1" ] && [ "${ccache_dir}" == "" ]; then echo -e "${CL_RED} * Error: ccache_dir not set [vendor/extra/config.sh]${CL_RST}\n"; else echo -e "${CL_GRN} * Setup ccache : ${CL_LBL}$c_current${CL_RST} of ${CL_LBL}$c_max${CL_RST} used in ${CL_LBL}$c_dir${CL_RST}"; fi
}
